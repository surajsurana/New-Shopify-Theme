...
- `.claude/agents/technical-director.md` -- Builds the actual Shopify theme (Liquid/JSON templates, sections, assets) from the approved spec. Does a feasibility skim against real Shopify-platform limits (not Dawn) before full build. Has read-only Shopify Admin API access (added 2026-07-12) plus `graphql_mutation` (granted directly by Suraj, 2026-08-11, after Claude Code's own permission-escalation check blocked a Claude-session-initiated attempt to add it). **Owns the full release pipeline: STAGING deploy, LIVE<->STAGING drift check/reconciliation, and -- only after Suraj's explicit per-release approval -- promoting to Live via git.** The `graphql_mutation` grant is STAGING-only in practice: the connected Shopify MCP server itself hard-blocks any theme-file write or publish mutation against the live/published theme regardless of which theme ID is passed, independent of this tool grant.
